...
import AreaInfoPanel from "@/app/components/AreaInfoPanel"
...
export default function Page() {
    ...
    const getColorForArea = useCallback((index: number) => {
        if (index === 0) return "blue"
        if (index === 1) return "red"
        const colors = ["green", "yellow", "cyan", "gray", "orange", "purple", "teal", "pink"]
    ...
    const handleFieldClick = useCallback((bounds: [[number, number], [number, number]], feature: any) => {
        if (analysisMode === "area") {
            const newArea = {
                ...
                geometry: feature,
                bounds: bounds,
                color: getColorForArea(selectedAreas.length),
    ...
    }, [setBounds, analysisMode, selectedAreas.length, getColorForArea])
    ...
    const handleFinalize = useCallback(() => {
        if (analysisMode === "area" && currentBounds) {
            const newArea = {
                id: `area_${Date.now()}_${Math.random()}`,
                geometry: null,
        ...
        setBoundsSource('rectangle')
        setSelectedFieldFeature(null)
    }, [finalizeRectangle, analysisMode, currentBounds, selectedAreas.length, getColorForArea])
