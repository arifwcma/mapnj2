...
export function getAnalyticsSummary(startDate = null, endDate = null) {
    ...
    const topEventsStmt = db.prepare(`
        ...
        FROM analytics 
        ${startDate || endDate ? 'WHERE ' + (startDate ? 'timestamp >= ?' : '') + (startDate && endDate ? ' AND ' : '') + (endDate ? 'timestamp <= ?' : '') : ''}
        GROUP BY event_type 
        ORDER BY count DESC 
        LIMIT 10
    `)
    const topEvents = topEventsStmt.all(...(startDate || endDate ? [startDate, endDate].filter(Boolean) : [])).map(row => ({
